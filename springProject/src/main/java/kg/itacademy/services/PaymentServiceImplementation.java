...
//    }
    ...
    @Override
    public List<Payment> getPaymentsByDateOfCustomerId(Date start, Date end, Long customerId) {

        return paymentRepository.findAllByDateOfClientId(start,end,customerId);
    }
    ...
    public Payment confirmPayment(ConfirmationModel confirmationModel, String code) throws Exception {
        ...
        if (!payment.getConfirmationCode().equals(confirmationModel.getConfirmationCode())) {
            Payment p = getById(confirmationModel.getPaymentId());
            Integer count = p.getCount() + 1;
            p.setCount(count);
            update(p);
            if (count>=3)
    ...
    private void processPayment(Payment payment) throws Exception {
        ...
        if (payment.getStatus().equals(Status.SUCCESS)&& !payment.getDone()) {
